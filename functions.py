...
async def languagePack(languageCode, request):
...
async def reistrationCheck(id, now):
...
async def saveLanguage(call):
    ...
    await reistrationCheck(id, now)
    # Save the language.
    cursor.execute("UPDATE users SET language_code = ? WHERE id = ?;", (languageCode, id))
    conn.commit()
    language_is_chosen = await languagePack(languageCode, "language_is_chosen")
